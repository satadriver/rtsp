...
int RtspClient::Authority(char* sendBuf, int sendLen, char* recvBuf, int recvLimit,int seq) {
	...
	if (seq == 1) {
		authLen = sprintf(strAuth, g_strDescribeAuth, method.c_str(), m_ip.c_str(), atoi(cseq.c_str()),
			user.c_str(), realm.c_str(), nonce.c_str(), uri.c_str(), codeMd5);
	}
	else if (seq == 2) {
		authLen = sprintf(strAuth, g_strSetupAuth, 
			method.c_str(), m_ip.c_str(), atoi(cseq.c_str()),
			user.c_str(), realm.c_str(), nonce.c_str(), uri.c_str(), codeMd5);
	}
	else if (seq == 3) {
...
int RtspClient::Client(const char * fn) {
	...
	for (seq = 0; seq < n; seq++) {
		...
		sendLen = 0;
		if (seq == 3) {
			sendLen = sprintf(sendBuf, format, m_ip.c_str(), cseq ++, (char*) m_session.c_str());
		}
		else if (seq == 2) {
			sendLen = sprintf(sendBuf, format, m_ip.c_str(), cseq++);
		}	
		else if (seq == 1) {
			sendLen = sprintf(sendBuf, format, m_ip.c_str(), cseq++);
		}
			...
			sendLen = sprintf(sendBuf, format, m_ip.c_str());
		}
